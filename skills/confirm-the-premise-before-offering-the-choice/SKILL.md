...
### 6. ⭐ **대가 칸도 주장이다** — 메커니즘만 재고 가격표를 안 재면 같은 사고가 난다 (2026-09-22)

이 스킬을 지켰는데도 한 번 더 되물림을 받았다. **선택지가 무엇을 하는지는 코드로 확인했고,
틀린 것은 "그게 얼마인가" 였다.**

BSVibe #1003, 제품별 배송 목적지. 낸 메뉴:

| 옵션 | 내가 적은 대가 |
|---|---|
| 바인딩에 **새 전용 키** (추천) | "마이그레이션 1건 + 리졸버 수정" |
| 계정을 나눈다 | "방 개수만큼 계정이 는다" |

형님은 **새 키**를 골랐다. 그 직후 prod 를 조회하니 — **그 키는 이미 존재했고, 세 행 모두
이미 채워져 있었고, 읽는 코드만 없었다.** 진짜 대가는 *마이그레이션 0건*이었고, 새로
만들었다면 **같은 주소의 네 번째 사본**이 생기고 기존 값은 고아가 될 참이었다.

⇒ 메뉴를 되돌리고 다시 물어야 했다. **사용자는 내가 적은 숫자로 결정한다.**

무엇이 달랐나: 나는 *코드*(selection 을 누가 읽나 → 인바운드가 읽는다, 참)는 쟀고
*데이터*(selection 에 지금 뭐가 들어 있나)는 안 쟀다.

* **"새로 만들어야 한다"는 대가는 특히 위험하다** — 그 주장이 참이려면
  **"이미 없다"를 세었어야** 하는데, 코드 grep 은 *독자*가 없다는 것만 증명한다.
  `[[a-populated-column-with-no-reader-is-a-missing-consumer]]`
* **비용은 코드가 아니라 데이터에 있을 수 있다.** 마이그레이션·백필·고아 데이터는
  전부 prod 행을 봐야 나오는 숫자다.
* **대가가 틀린 쪽으로 틀리는 방향을 보라** — 오늘은 추천안이 *말한 것보다 쌌다*.
  싸게 틀려도 사고다. 사용자가 "그만한 값어치는 없겠다"로 **안 골랐을 수도** 있다.
...
- [ ] ⭐ **각 옵션의 대가(마이그레이션·백필·새 키·되돌리기 비용)를 코드가 아니라
      데이터로 확인했다** — 특히 "새로 만들어야 한다"는 대가는 "이미 없다"를 센 결과인가
...
- `a-populated-column-with-no-reader-is-a-missing-consumer` — "새 키가 필요하다"는
  대가가 틀리는 가장 흔한 방식
