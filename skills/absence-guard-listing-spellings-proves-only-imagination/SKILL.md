...
## 사례 — 산문과 언어 경계 (2026-09-22, BSVibe `trigger.enabled`)

기존 가드가 **파일 목록은 제대로** 들고 있었는데도 결함을 놓쳤다. 놓친 이유가 둘이다.

1. **철자 하나만 봤다.** `'"enabled"'`(쌍따옴표 포함)만 검사해서, 툴 설명의
   산문형 `({enabled, filters})` 를 통과시켰다. 그 문장이 **에이전트가 읽는 메뉴**였고
   실제 생산자였다. → [[a-class-docstring-is-the-agent-facing-menu]]
2. **표면 목록이 파이썬뿐이었다.** 같은 키를 쓰는 프런트엔드(TS)가 목록에 없었고,
   거기서 컨트롤 하나가 11일간 422 를 뱉고 있었다. **한 언어로 쓴 가드는 다른 언어에
   대해 아무 말도 안 한다.**

고칠 때 세 번 더 걸렸다 — 전부 "가드가 틀린 이유로 실패"하는 형태다:

| 시도 | 왜 틀렸나 |
|---|---|
| 철자 목록을 넓혔다 | **자기 설명 문장**(독스트링의 제거 내력)에 걸려 빨개졌다 |
| 주석·독스트링을 걷어냈다 | `enabled:` 가 무관한 `quiet_hours_enabled:` 에 걸렸다 |
| 단어 경계를 넣었다 | 양성 대조군이 *"모든 표면 파일이 `filters` 를 코드에 쓴다"* 는 **거짓 명제**였다 |

최종형:

```python
# 산문은 폐기를 narrate 해도 되고, 코드는 그 키를 쓰면 안 된다
spellings = (r'"enabled"', r"'enabled'", r"\benabled\s*:", r"\benabled\s*=",
             r"\{enabled\b", r"\.enabled\b")          # 단어 경계 필수
offenders = {p.name: [x for x in spellings if re.search(x, _code_only(p.read_text()))] ...}
```

그리고 **스트리퍼 자체에 양방향 대조군**을 붙였다 — 프로즈와 코드를 둘 다 담은 probe 를
넣어 *"코드는 살아남고 산문은 전부 사라진다"* 를 단언한다. 이게 없으면 스트리퍼가
파일을 통째로 삼켜도 가드는 영원히 초록이다.

⚠️ **대조군의 명제를 아무거나 쓰지 마라.** "모든 파일이 X 를 쓴다" 같은 편한 명제는
리팩터 한 번에 거짓이 되고, 그때 사람들은 **대조군을 지운다.** 대조군은 **검사 도구
자신**을 겨냥하는 게 가장 오래 산다.
